...
namespace Com.Kawaiisun.SimpleHostile
{
    ...
    public class Manager : MonoBehaviourPunCallbacks, IOnEventCallback
    {
        ...
        public void CheckWinner() 
        {
            ...
            for (int i = 0; i < playerInfo.Count; i++)
            {
                if(playerInfo[i].kills > KillsWinner)
                {
                    usernameWinner = playerInfo[i].profile.username;
                    KillsWinner = playerInfo[i].kills;
                }
                else if (playerInfo[i].kills ==KillsWinner)
                {
                    usernameWinner = "Draw";
                }
            }
            StartCoroutine(Give.UpdateMoneyValue(usernameWinner));
            
            if (usernameWinner == "Draw")
            {
                winnerText.text = usernameWinner;
            }
            else
            {
                StartCoroutine(Give.UpdateMoneyValue(usernameWinner));
                winnerText.text = usernameWinner+" Winner";
            }
        }

                ...
                if (Physics.Raycast(t_spawn.position, t_bloom, out t_hit, 1000f, canBeShot))
                {
                    ...
                    if (photonView.IsMine)
                    {
                        if(t_hit.transform.tag == "Animal")
                        {
                            ...
                            hitmarkerWait = 1f;
                            Debug.Log("AnimalHit");
                            AnimalsController Controller = t_hit.collider.transform.root.gameObject.GetComponent<AnimalsController>();
                            if (Controller != null)
                            {
                                Debug.Log("Got");
                            }
                            t_hit.collider.transform.root.gameObject.GetPhotonView().RPC("TakeDamageAnimal", RpcTarget.All, loadout[currentIndex].damage, PhotonNetwork.LocalPlayer.ActorNumber);
